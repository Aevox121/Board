...
核心能力：
- **画布**：基于 Excalidraw 的完整绘图能力（手绘 / 图形 / 连线 / 文本）。
...
| 层 | 选型 |
|----|------|
| 绘图内核 | `@excalidraw/excalidraw`（开源包） |
| 内容元素层 | React + DOM 覆盖层（与画布共用坐标系，渲染文件/文件夹/区域/Markdown 卡片） |
...
**当前阶段：M0**——优先验证 Excalidraw 自定义元素 / DOM 覆盖层可行性、文件夹↔画布同步原型，据此定稿选型。
...
### 关键风险
- Excalidraw 对自定义元素类型支持有限 → 文件/文件夹/区域大概率走"DOM 覆盖层 + 自有数据模型"，M0 须验证。
- 文件夹↔画布双向同步的一致性边界 → M0 出原型，明确冲突规则。

### 已知技术债
- **Node 包 ESM 产物扩展名**：`tsconfig.base.json` 用 `moduleResolution: Bundler`，编译产物的相对 import 不带 `.js` 扩展名，Node 原生 ESM 无法直接运行 `dist/`。当前 `@board/cli` 用 `src/loader.ts` 解析钩子兜底、`@board/server` 经 `tsx` 运行。后续应给 Node 侧（core/server/cli）源码相对 import 补全 `.js` 扩展名以根治。
